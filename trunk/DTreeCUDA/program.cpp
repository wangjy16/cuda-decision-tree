...
void build_full_tree_from_file_and_dfs_travel()
{
    ...
    Node* node = new Node;
    t.iniNode(node);
    t.build(node, t.transactions, t.transactionCount, t.columns, t.columnCount, 1);
    cout << "Parent  \t" << "Option  \t" << "Current \t" << "Gain Info\t" << "Trans Count" << endl;
    t.DFSTravel(node);
}
...
void gpu_cpu_build_compare()
{
    ...
    int column_count = 3;
    int transaction_count = 1000 * 1000 * 5;
    Column* columns = t.generateColumn(column_count, 2, 2);
    Transaction* transactions = t.generateTransaction(columns, column_count, transaction_count);
    // GPU building
    gpu_node = new Node;
    ...
    gpu_time = clock() - start;
    cout << "GPU building time: " << (float) gpu_time / CLOCKS_PER_SEC << " (s)" << endl;
    // CPU building
    cpu_node = new Node;
    t.iniNode(cpu_node);
    start = clock();
    ...
    cout << "CPU building time: " << (float) cpu_time / CLOCKS_PER_SEC << " (s)" << endl;
    // Compare
    cout << "Speedup: " << (float) cpu_time / (float) gpu_time << endl;
    cout << "Parent  \t" << "Option  \t" << "Current \t" << "Gain Info\t" << "Trans Count" << endl;
    t.DFSTravel(cpu_node);
}
